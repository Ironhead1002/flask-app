...
def register():
    ...
    if form.validate_on_submit():
        ...
        user = User(username=form.username.data, email=form.email.data, password=str(hashed_password))
        db.session.add(user)
        db.session.commit()
        a = User.query.all()
        print(a)
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash('An email has been sent with instruction to reset password')
        return redirect(url_for('users.login'))
